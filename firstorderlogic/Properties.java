...
public class Properties {
	...
	public static boolean IsDead(SingleCell s) {
		boolean result = false;
		if (s.getNumberOfPests() > 15 || s.getIrrigation() == 0 || s.getSoilDestruction() == 100)
			result = true;
		else
			result = false;
		return result;
	...
	public static boolean IsinPerfectCondidion(SingleCell s) {
		boolean result = false;
		if (ClearfromPests(s) == true && WellWatered(s) == true && WellFertilised(s) == true)
			result = true;
		else
			result = false;
		return result;
	...
	public static boolean SomeWorkNeeded(SingleCell s) {
		boolean result = false;
		if (ClearfromPests(s) == false && WellWatered(s) == true && WellFertilised(s) == true)
			result = true;
		if (ClearfromPests(s) == true && WellWatered(s) == false && WellFertilised(s) == true)
			result = true;
		if (ClearfromPests(s) == true && WellWatered(s) == true && WellFertilised(s) == false)
			result = true;
		else
			result = false;
		return result;
	}
	// Mo¿na definiowaæ przez logiczne || i && w Javie, albo korzystaæ z operatorów
	// z Operators.
	// W miare potrzeb mo¿na zdefiniowaæ za pomoc¹ prostych andów relacje typu XOR,
	// NAND nawet
	// w operatorach.
}
